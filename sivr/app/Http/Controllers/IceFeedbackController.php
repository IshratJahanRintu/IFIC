...
class IceFeedbackController extends Controller
{
    ...
    public function storeIceFeedback(Request $request)
    {
        ...
        if ($this->cache->hasCacheValue('sessionId' . $cacheKey) && $this->cache->getCacheData('startTime' . $cacheKey)) {
            ...
            if ($this->dataLogger->storeCustomerFeedback($stopTime, $timeInIvr, $sessionId, $feedback)) {
                $this->cache->removeCacheData('sessionId' . $cacheKey);
                $this->cache->removeCacheData('startTime' . $cacheKey);
